(function ($) {
  ...
  /*---------- 01. On Load Function ----------*/
    $(window).on("load", function () {
        // Tibra: target by ID to avoid re-triggering on SPA navigation
        var $pre = $("#tibra-preloader");
        if ($pre.length && $pre.css("display") !== "none") {
            $pre.fadeOut(400);
        }
        wowAnimation();
    });
    ...
    /*---------- 02. Preloader ----------*/
    // Tibra: cancel button removed — auto-dismissed by RouteAnimationReinit
    if ($("#tibra-preloader").length > 0) {
        $(".preloaderCls").on("click", function (e) {
            e.preventDefault();
            $("#tibra-preloader").hide();
        });
    }
